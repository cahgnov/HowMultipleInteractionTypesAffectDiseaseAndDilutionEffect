function dydt = model_ode_den(t,y,n,beta,r,nu,matrix_c,matrix_p,matrix_m,h,delta,rho)
...
matrix_P2 = matrix_P1';%matrix_P2_{i,j}=1,represents species j prey i.
...
dSi=Si.*(r-delta.*H+sum_C+term_mutualism+term_predation)+...
    (Si./(h+H)).*term_prey-Si.*(beta*Ii)+rho.*Ii;
dIi=Ii.*(r-delta.*H+sum_C+term_mutualism+term_predation)+...
    (Ii./(h+H)).*term_prey+Si.*(beta*Ii)+nu.*Ii-rho.*Ii;
